...
public final class DesUtils {
    ...
    private static byte[] hex2byte(byte[] b) {
        if ((b.length % 2) != 0) throw new IllegalArgumentException("length not even");
    ...
    private static String encode(String src, String key) {
        ...
        if (bytes != null) encryptStr = byte2hex(bytes);
        return encryptStr;
    }

    public static void main(String[] args) {
        String ss = "uu123@#$";
        String KEY = "4YztMHI7PsT4rLZN";
        String encodeSS = encode(ss, KEY);
        System.out.println(encodeSS);
        String decodeSS = decode(encodeSS, KEY);
